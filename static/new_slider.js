...
var min_val = Date.parse(dt_from) / 1000;  // No division by 3600
var max_val = Date.parse(dt_to) / 1000;  // No division by 3600
...
function updateForecastDates(ui) {
        ...
        console.log('updateForecastDates called with ui = ', ui); // additional logging to see what's in the ui object when the function is called

    // Check if the ui argument is an event
    if (ui instanceof Event) {
        // Handle the event case
        // For example, you might get the values from the event's target
        var values = $(ui.target).slider('values');
        if (values.length < 2) {
            console.error("updateForecastDates was called with an invalid argument");
            return;
        }
        var dt_cur_from = new Date(values[0] * 1000 * 3600);
        var dt_cur_to = new Date(values[1] * 1000 * 3600);
    } else {
        ...
        console.log('UI slider values:', ui.values);  // Add this line
    }
...
$("#slider-range").slider({
    ...
    max: max_val,
    step: 1,
    values: [initial_from.getTime() / 1000, initial_to.getTime() / 1000],
    slide: function (e, ui) {
        var dt_cur_from = new Date(ui.values[0] * 1000);  // No multiplication by 3600
        $('.slider-time').html(formatDT(dt_cur_from));

        var dt_cur_to = new Date(ui.values[1] * 1000);  // No multiplication by 3600
